...
function Card(props, ref) {
	...
	return (
		<div className="w-[600px] h-[600px] bg-[#4241FF] p-8 m-auto" ref={ref}>
			<div className="flex flex-col gap-3 items-left text-black p-4 h-full bg-[#7270FF] rounded-xl border-2 border-black font-medium">
				<div className="flex items-top gap-4">
					...
					<div className="w-fit bg-white rounded-t-xl p-2 px-4 border-2 border-black w-fit flex items-center gap-8">
						#DEVResolutions2024
						<X />
					</div>
				</div>
				<div className="w-full bg-white rounded-xl p-1 px-4 border-2 border-black">
					<p>dev.to</p>
				</div>
				<div className="flex flex-col items-left justify-around h-full w-full bg-black text-white rounded-xl p-2 px-6">
					...
					<div className="w-full h-[2px] rounded-full bg-white -my-4" />
					{goals.map((title) => (
						<Goal text={title} />
					))}
				</div>
...
function Goal({ text }) {
	return (
		<div className="flex items-center gap-8 px-2">
			<div className="w-10 h-10 aspect-square rounded-md border-2 border-white" />
			<p className="text-3xl font-light">{text}</p>
		</div>
	);
}
